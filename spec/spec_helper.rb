...
RSpec.configure do |config|
  ...
  config.before(:all) do
    begin
      PG.connect(host:"postgres-proj", user: "admin", password: "password").exec("CREATE DATABASE rebase_challenge_test")
    rescue
    end

    connect_pg_test = PG.connect(host:"postgres-proj", user: "admin", password: "password", dbname: 'rebase_challenge_test')
  ...
  config.after(:all) do
    connect_pg_test = PG.connect(host:"postgres-proj", user: "admin", password: "password")
    connect_pg_test.exec("SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE datname = 'rebase_challenge_test';")
    connect_pg_test.exec("DROP DATABASE rebase_challenge_test")
    connect_pg_test.close
  end
